...
public:
    ...
    {
        // Declare parameters
        this->declare_parameter<std::string>("network_interface", "enp108s0");
        this->declare_parameter<std::string>("g1_loco_client_path", "/home/mhw/robot_g1/unitree_sdk2/build/bin/g1_loco_client");
        this->declare_parameter<std::string>("unitree_sdk_lib_path", "/home/mhw/robot_g1/unitree_sdk2/thirdparty/lib/x86_64");
        ...
        this->get_parameter("unitree_sdk_lib_path", unitree_sdk_lib_path_);
        ...
        RCLCPP_INFO(this->get_logger(), "Unitree SDK lib path: %s", unitree_sdk_lib_path_.c_str());
...
private:
    ...
    void cmdVelCallback(const geometry_msgs::msg::TwistStamped::SharedPtr msg)
    {
        ...
        double linear_x = clamp(msg->twist.linear.x, max_x_linear_vel_);
        double linear_y = clamp(msg->twist.linear.y, max_y_linear_vel_);
        double angular_z = clamp(msg->twist.angular.z, max_angular_vel_);
        
        // Log if velocities were clamped
        if (std::abs(msg->twist.linear.x) > max_x_linear_vel_ ||
            std::abs(msg->twist.linear.y) > max_y_linear_vel_ ||
            std::abs(msg->twist.angular.z) > max_angular_vel_)
        {
            RCLCPP_WARN(this->get_logger(), 
                "Velocity clamped! Requested: [%.2f, %.2f, %.2f]",
        ...
        if (!unitree_sdk_lib_path_.empty() && std::filesystem::exists(unitree_sdk_lib_path_)) {
            command << "env LD_LIBRARY_PATH=" << unitree_sdk_lib_path_ << ":${LD_LIBRARY_PATH:-} ";
        }
    ...
    std::string unitree_sdk_lib_path_;
